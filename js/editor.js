...
(function () {
  ...
  /* ==============================================================
     ...
     Also includes B (bold), A+ (large x1.3), and x strip buttons.
  ============================================================== */
  ...
  function _wireEffectFontPicker() {
    ...
    /* -- Helper: prevent blur on click (but allow select/input elements to work) -- */
    toolbar.addEventListener('mousedown', e => {
      // Don't prevent default on interactive elements -- SELECT needs it to open the
      // dropdown, and INPUT needs it to receive focus and keyboard events.
    ...
    sizeInput.tabIndex = 0;
    sizeInput.setAttribute('inputmode', 'numeric');
    sizeInput.setAttribute('pattern', '[0-9]*');
    ...
    _mkFmtBtn('A+', 'Large (x1.3) -- wraps in [[large:...]]', () => _wrapSelection('[[large:', ']]'));
    _mkFmtBtn('x strip', 'Remove [[tag:...]] nearest cursor', _stripTag);
    ...
    hint.innerHTML =
      ...
      '<code style="color:var(--c-accent,#c8ff00)">[[icon:1:]]</code>. ' +
      'Use <code>\\n</code> for line break.';
  }

  /* ==============================================================
     EFFECT ICON SLOTS (buff / debuff icons in card effect text)
     -------------------------------------------------------------
     Up to N icon slots. Each slot has:
       • An image upload (renders as [[icon:N:]] tag in the effect)
       • + / - buttons to add/remove slots
     The icon height in the renderer matches the Card Effect font size.
     Width = height (square, let the image's natural ratio clip via
     object-fit:contain inside the renderer).

     Tag syntax inserted into the textarea:  [[icon:1:]]
     The layers.js renderer resolves [[icon:N:]] to
       NullCorps.state.effectIcons[N-1].dataUrl
     and draws it at native effect-font-line-height × 1 square.
  ============================================================== */

  function _wireEffectIcons() {
    /* ── find or create the icon section container ── */
    const effectSection = document.getElementById('section-effect');
    if (!effectSection) return;

    /* Wrap everything in one field-group */
    const group = document.createElement('div');
    group.id = 'effect-icon-group';
    group.style.cssText = 'display:flex;flex-direction:column;gap:6px;margin-top:4px;';

    /* Section label */
    const lbl = document.createElement('label');
    lbl.textContent = 'Effect Icons';
    lbl.style.cssText = 'font-size:11px;font-family:var(--font-mono,monospace);letter-spacing:0.06em;color:var(--c-text-label,#8888a0);text-transform:uppercase;';
    group.appendChild(lbl);

    const hintEl = document.createElement('div');
    hintEl.style.cssText = 'font-size:9px;font-family:var(--font-mono,monospace);color:var(--c-text-hint,#555568);line-height:1.5;';
    hintEl.innerHTML = 'Upload icons, then click <b>Insert</b> to place <code style="color:var(--c-accent,#c8ff00)">[[icon:N:]]</code> at the cursor. ' +
      'Icon height = Card Effect font size. Width = height.';
    ...
    /* Append the group into the effect section */
    effectSection.appendChild(group);

    /* ── State: array of { dataUrl, filename } ── */
    ...
    /* ── Render the icon row list from state ── */
    function _renderList() {
      ...
      icons.forEach((icon, idx) => {
        ...
        /* Upload button */
        const upBtn = _mkIconBtn('↑ Upload', `Upload image for icon #${idx + 1}`, () => fileInput.click());
        row.appendChild(upBtn);

        /* Insert-into-textarea button */
        const textarea = document.getElementById('card-effect');
        const insertBtn = _mkIconBtn(`Insert [[icon:${idx + 1}:]]`, `Insert [[icon:${idx + 1}:]] at cursor in effect text`, () => {
          if (!textarea) return;
          const tag = `[[icon:${idx + 1}:]]`;
          const start = textarea.selectionStart;
          const end   = textarea.selectionEnd;
          const val   = textarea.value;
        ...
        insertBtn.style.background = 'var(--c-accent-dim,rgba(200,255,0,0.12))';
        insertBtn.style.borderColor = 'rgba(200,255,0,0.3)';
        insertBtn.style.color = 'var(--c-accent,#c8ff00)';
        row.appendChild(insertBtn);

        /* Remove slot button */
        const removeBtn = _mkIconBtn('✕', `Remove icon slot #${idx + 1}`, () => {
          icons.splice(idx, 1);
          _triggerRender();
          _renderList();
        });
      ...
      /* Show/hide add button hint when list is empty */
      hintEl.style.display = icons.length === 0 ? '' : '';
    }

    /* ── Helper: small utility button ── */
    function _mkIconBtn(text, title, handler) {
      const b = document.createElement('button');
      b.type = 'button'; b.textContent = text; b.title = title;
      b.style.cssText = [
        ...
        'background:var(--c-surface-2,#1e1e2e)', 'color:var(--c-text,#e0e0f0)',
        'border:1px solid var(--c-border,#333348)', 'border-radius:3px',
        'cursor:pointer', 'white-space:nowrap', 'line-height:1.8', 'flex-shrink:0',
      ].join(';');
      b.addEventListener('mouseover', () => { if (!b.dataset.accent) b.style.background = 'var(--c-surface-3,#2a2a3e)'; });
      b.addEventListener('mouseout',  () => { if (!b.dataset.accent) b.style.background = 'var(--c-surface-2,#1e1e2e)'; });
      b.addEventListener('click', handler);
      return b;
    }

    /* ── Wire add-slot button ── */
    ...
    /* ── Expose so presets can serialise icon state ── */
  ...
  /* ==============================================================
     ...
     Mirrors the checkbox in the identity section to crossword.js.
  ============================================================== */
  ...
  /**
   * Return serialisable icon state for preset saving.
   * Each entry: { dataUrl, filename }
   */
  ...
  /**
   * Restore icon state from a preset snapshot.
   * @param {Array} snap  -- same shape as getIconState() output
   */
  function applyIconState(snap) {
    ...
    // Re-render the icon list UI by tearing down and rebuilding the group
