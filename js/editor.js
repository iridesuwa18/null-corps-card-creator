/* ===============================================================
   NULL CORPS CARD EDITOR -- js/editor.js
   Part 4: Text Layer Editor
   -------------------------------------------------------------
   Wires all settings-panel inputs to NullCorps.state and triggers
   a layers.js re-render on every change.

   Text layers handled:
     Layer 9  * Creator Credit    (TGL Engschrift 9pt, white/black 4px)
     Layer 10 * Game Name         (TGL Engschrift 9pt, white/black 4px)
     Layer 11 * Unique Card #     (TGL Engschrift 9pt, white/black 4px)
     Layer 12 * ATK Stat          (TGL Engschrift 50pt, white/black 4px)
     Layer 13 * DEF Stat          (TGL Engschrift 50pt, white/black 4px)
     Layer 14 * HP Stat           (TGL Engschrift 32pt, black/white 4px)
     Layer 15 * SHD Stat          (TGL Engschrift 32pt, black/white 4px)
     Layer 16 * Energy Stat       (TGL Engschrift 50pt, white/black 4px)
     Layer 1  * Zone Indicator    (TGL Engschrift 50pt, white/black 4px)
     Layer 26 * Hierarchy Text    (TGL Engschrift 21pt, black/white 4px)
     Layer 27 * Card Title        (TGL Engschrift 21pt, white/black 4px, confined)
     Layer 28 * Card Type Text    (TGL Engschrift 21pt, black/white 4px)
     Layer 29 * Card Effect Desc  (Archivo ExtraCondensed Light 17pt, black/white 4px,
                                   confined 2000.7x285.8px)

   All other fields (card name, crossword, tile URL) write to state
   and trigger render -- crossword.js reads them separately.
   =============================================================== */

(function () {
  'use strict';

  /* ==============================================================
     FIELD MAP
     Maps each input element ID -> the NullCorps.state key it drives.
     Type 'number' coerces to Number; 'text' keeps as string.
  ============================================================== */
  const FIELD_MAP = [
    /* Identity & meta */
    { id: 'card-name',       stateKey: 'cardName',       type: 'text'   },
    { id: 'card-title',      stateKey: 'cardTitle',      type: 'text'   },
    { id: 'card-type',       stateKey: 'cardType',       type: 'text'   },
    { id: 'era-text',        stateKey: 'era',            type: 'text'   },
    { id: 'unique-number',   stateKey: 'uniqueNumber',   type: 'text'   },
    { id: 'card-effect',     stateKey: 'cardEffect',     type: 'text'   },
    /* Stats */
    { id: 'stat-atk',        stateKey: 'atk',            type: 'number' },
    { id: 'stat-def',        stateKey: 'def',            type: 'number' },
    { id: 'stat-hp',         stateKey: 'hp',             type: 'dynamic' },
    { id: 'stat-shd',        stateKey: 'shd',            type: 'dynamic' },
    { id: 'stat-energy',     stateKey: 'energy',         type: 'number' },
    /* Creator */
    { id: 'creator-credit',  stateKey: 'creatorCredit',  type: 'text'   },
    { id: 'game-name',       stateKey: 'gameName',       type: 'text'   },
    /* Crossword tile URL */
    { id: 'tile-image-url',  stateKey: 'tileImageUrl',   type: 'text'   },
    /* Zone indicator is wired by territories.js -- we still sync state here on manual edit */
    { id: 'zone-indicator',  stateKey: 'zone',           type: 'text'   },
    /* Symbol position & size */
    { id: 'territory-symbol-x',    stateKey: 'territorySymbolX',    type: 'number' },
    { id: 'territory-symbol-y',    stateKey: 'territorySymbolY',    type: 'number' },
    { id: 'territory-symbol-size', stateKey: 'territorySymbolSize', type: 'number' },
    { id: 'energy-symbol-x',       stateKey: 'energySymbolX',       type: 'number' },
    { id: 'energy-symbol-y',       stateKey: 'energySymbolY',       type: 'number' },
    { id: 'energy-symbol-size',    stateKey: 'energySymbolSize',    type: 'number' },
  ];

  /* ==============================================================
     HELPERS
  ============================================================== */

  /** Trigger a full layer re-render if layers.js is ready. */
  function _triggerRender() {
    if (window.NullCorps.layers && typeof window.NullCorps.layers.render === 'function') {
      window.NullCorps.layers.render();
    }
  }

  /** Trigger crossword rebuild if crossword.js is ready. */
  function _triggerCrossword() {
    if (window.NullCorps.crossword && typeof window.NullCorps.crossword.build === 'function') {
      window.NullCorps.crossword.build();
    }
  }

  /**
   * Sync a single input element's visual value from NullCorps.state.
   * Used when a preset is loaded to push state -> UI.
   */
  function _syncInputFromState(fieldDef) {
    const el = document.getElementById(fieldDef.id);
    if (!el) return;
    const val = window.NullCorps.state[fieldDef.stateKey];
    if (val === undefined || val === null) return;

    if (el.tagName === 'SELECT') {
      el.value = String(val);
    } else {
      // 'dynamic' fields: treat as text in char-skill, number otherwise
      const effectiveType = fieldDef.type === 'dynamic'
        ? (window.NullCorps.state.cardMode === 'char-skill' ? 'text' : 'number')
        : fieldDef.type;
      el.value = effectiveType === 'number'
        ? (val === 0 ? '' : String(val))
        : String(val);
    }
  }

  /* ==============================================================
     EVENT WIRING
  ============================================================== */

  function _wireFields() {
    for (const field of FIELD_MAP) {
      const el = document.getElementById(field.id);
      if (!el) continue;

      const eventType = (el.tagName === 'SELECT') ? 'change' : 'input';

      el.addEventListener(eventType, function () {
        const raw = this.value;
        let val;

        // 'dynamic' fields behave as text in char-skill mode, number otherwise
        const effectiveType = field.type === 'dynamic'
          ? (window.NullCorps.state.cardMode === 'char-skill' ? 'text' : 'number')
          : field.type;

        if (effectiveType === 'number') {
          val = raw === '' ? 0 : Number(raw);
          if (isNaN(val)) val = 0;
        } else {
          val = raw;
        }

        window.NullCorps.state[field.stateKey] = val;

        // Card name changes also need a crossword rebuild
        if (field.id === 'card-name') {
          _triggerCrossword();
        }

        _triggerRender();
      });
    }
  }

  /* ==============================================================
     STATE -> UI SYNC
     Call this after a preset is loaded to push all state values
     back into the input elements.
  ============================================================== */

  /**
   * Sync all managed input fields from current NullCorps.state.
   * Does NOT trigger a render -- the caller should do that if needed.
   */
  function syncFromState() {
    for (const field of FIELD_MAP) {
      _syncInputFromState(field);
    }
  }

  /* ==============================================================
     PRESET INTEGRATION
     Expose a helper so js/presets.js can easily rebuild the full
     state snapshot (text fields only).
  ============================================================== */

  /**
   * Return an object with all text-layer state values keyed by their
   * state key. Used when serialising a preset to JSON.
   */
  function getTextState() {
    const s = window.NullCorps.state;
    const isCharSkill = s.cardMode === 'char-skill';
    return {
      cardName:      s.cardName      || '',
      cardTitle:     s.cardTitle     || '',
      cardType:      s.cardType      || '',
      era:           s.era           || '',
      uniqueNumber:  s.uniqueNumber  || '',
      cardEffect:    s.cardEffect    || '',
      atk:           s.atk           ?? 0,
      def:           s.def           ?? 0,
      hp:            isCharSkill ? (s.hp  ?? '') : (s.hp  ?? 0),
      shd:           isCharSkill ? (s.shd ?? '') : (s.shd ?? 0),
      energy:        s.energy        ?? 0,
      creatorCredit: s.creatorCredit || 'iridesuwa',
      gameName:      s.gameName      || 'Null Corps',
      tileImageUrl:  s.tileImageUrl  || '',
    };
  }

  /**
   * Apply a text-state snapshot (e.g. from a loaded preset) into
   * NullCorps.state and sync all inputs.
   *
   * @param {object} snap -- same shape as getTextState() output
   */
  function applyTextState(snap) {
    if (!snap) return;
    const s = window.NullCorps.state;
    const alwaysNumberKeys = ['atk', 'def', 'energy'];
    // hp and shd are text in char-skill mode (Exchange Skill 1 / 2)
    const isCharSkill = s.cardMode === 'char-skill';

    for (const key of Object.keys(snap)) {
      if (alwaysNumberKeys.includes(key)) {
        s[key] = Number(snap[key]) || 0;
      } else if ((key === 'hp' || key === 'shd') && !isCharSkill) {
        s[key] = Number(snap[key]) || 0;
      } else {
        s[key] = snap[key] ?? '';
      }
    }

    syncFromState();
  }

  /* ==============================================================
     CHARACTER COUNTER FOR CARD EFFECT (optional UX enhancement)
     The confined box is 2000.7 x 285.8 px at native card size.
     At the rendering scale it's tight -- a live char counter helps.
  ============================================================== */

  function _wireEffectCounter() {
    const textarea = document.getElementById('card-effect');
    if (!textarea) return;

    // Insert a small counter below the textarea
    const counter = document.createElement('div');
    counter.id = 'card-effect-counter';
    counter.style.cssText = [
      'font-size: 10px',
      'font-family: var(--font-mono, monospace)',
      'color: var(--c-text-hint, #555568)',
      'text-align: right',
      'margin-top: -2px',
      'letter-spacing: 0.04em',
    ].join(';');

    // Insert after the hint div if present, otherwise after the textarea
    const parent = textarea.parentElement;
    parent.appendChild(counter);

    function _update() {
      const len = textarea.value.length;
      counter.textContent = `${len} char${len !== 1 ? 's' : ''}`;
      // Subtle warning colour above ~300 chars (box gets crowded)
      counter.style.color = len > 300
        ? 'var(--c-accent, #c8ff00)'
        : 'var(--c-text-hint, #555568)';
    }

    textarea.addEventListener('input', _update);
    _update();
  }

  /* ==============================================================
     CARD ART UPLOAD (Layer 36)
     Reads the chosen file as a data URL, stores it in state, and
     triggers a render.  Also wires the x Remove button.
  ============================================================== */

  function _wireCardArt() {
    const fileInput   = document.getElementById('card-art-input');
    const removeBtn   = document.getElementById('btn-remove-card-art');
    const filenameEl  = document.getElementById('card-art-filename');
    const previewWrap = document.getElementById('card-art-preview-wrap');
    const previewImg  = document.getElementById('card-art-preview');

    if (!fileInput) return;

    fileInput.addEventListener('change', function () {
      const file = this.files && this.files[0];
      if (!file) return;

      const reader = new FileReader();
      reader.onload = function (e) {
        const dataUrl = e.target.result;
        window.NullCorps.state.cardArtDataUrl = dataUrl;

        // Update UI
        if (filenameEl)  filenameEl.textContent = file.name;
        if (removeBtn)   removeBtn.style.display = '';
        if (previewImg)  previewImg.src = dataUrl;
        if (previewWrap) previewWrap.style.display = '';

        _triggerRender();
      };
      reader.readAsDataURL(file);
    });

    if (removeBtn) {
      removeBtn.addEventListener('click', function () {
        _clearCardArt(fileInput, filenameEl, removeBtn, previewWrap, previewImg);
      });
    }
  }

  function _clearCardArt(fileInput, filenameEl, removeBtn, previewWrap, previewImg) {
    window.NullCorps.state.cardArtDataUrl = '';
    if (fileInput)   fileInput.value = '';
    if (filenameEl)  filenameEl.textContent = 'No file chosen';
    if (removeBtn)   removeBtn.style.display = 'none';
    if (previewWrap) previewWrap.style.display = 'none';
    if (previewImg)  previewImg.removeAttribute('src');
    _triggerRender();
  }


  /* ==============================================================
     CARD EFFECT EDITOR  (Layer 29)
     ─────────────────────────────────────────────────────────────
     Simple syntax:
       [TGL text]   → TGL Engschrift font
       plain text   → Archivo ExtraCondensed Light (default)
       <Img(N)>     → icon slot N (leave a square gap, png overlaid)

     Controls:
       • Font size input (pt) — applies to the whole box
       • Textarea for effect text with Insert buttons
       • Icon slots: upload PNGs, click Insert to place <Img(N)>
  ============================================================== */

  function _wireEffectEditor() {
    const section = document.getElementById('section-effect');
    if (!section) return;

    const textarea = document.getElementById('card-effect');
    if (!textarea) return;

    /* ── Font size control ── */
    const sizeRow = document.createElement('div');
    sizeRow.style.cssText = 'display:flex;align-items:center;gap:8px;margin-bottom:6px;';

    const sizeLabel = document.createElement('label');
    sizeLabel.textContent = 'Font Size (pt)';
    sizeLabel.style.cssText = 'font-size:11px;font-family:var(--font-mono,monospace);color:var(--c-text-label,#8888a0);white-space:nowrap;';
    sizeRow.appendChild(sizeLabel);

    const sizeInput = document.createElement('input');
    sizeInput.type = 'number';
    sizeInput.id   = 'effect-font-size-pt';
    sizeInput.min  = 6; sizeInput.max = 80; sizeInput.step = 1;
    sizeInput.value = window.NullCorps.state.effectFontSize || 23;
    sizeInput.style.cssText = [
      'width:56px', 'font-size:12px',
      'background:var(--c-surface-3,#2a2a3e)', 'color:var(--c-text,#e0e0f0)',
      'border:1px solid var(--c-border,#333348)', 'border-radius:3px',
      'padding:2px 6px', 'font-family:var(--font-mono,monospace)',
    ].join(';');
    sizeInput.addEventListener('input', function () {
      const v = parseInt(this.value, 10);
      if (!isNaN(v) && v >= 6 && v <= 80) {
        window.NullCorps.state.effectFontSize = v;
        _triggerRender();
      }
    });
    sizeRow.appendChild(sizeInput);

    /* Insert before the field-group containing the textarea */
    const fieldGroup = textarea.closest('.field-group') || textarea.parentElement;
    fieldGroup.parentElement.insertBefore(sizeRow, fieldGroup);

    /* ── Syntax hint ── */
    const hint = document.createElement('div');
    hint.style.cssText = 'font-size:9px;font-family:var(--font-mono,monospace);color:var(--c-text-hint,#555568);margin-top:3px;line-height:1.7;';
    hint.innerHTML =
      '<b style="color:var(--c-accent,#c8ff00)">[TGL text]</b> → TGL font &nbsp;|&nbsp; ' +
      'plain text → Archivo Light &nbsp;|&nbsp; ' +
      '<b style="color:var(--c-accent,#c8ff00)">&lt;Img(N)&gt;</b> → icon N &nbsp;|&nbsp; ' +
      'new line → line break';
    fieldGroup.appendChild(hint);

    /* ── Character counter ── */
    const counter = document.createElement('div');
    counter.style.cssText = 'font-size:10px;font-family:var(--font-mono,monospace);color:var(--c-text-hint,#555568);text-align:right;margin-top:-1px;';
    fieldGroup.appendChild(counter);
    function _updateCounter() {
      const len = textarea.value.length;
      counter.textContent = `${len} char${len !== 1 ? 's' : ''}`;
      counter.style.color = len > 300 ? 'var(--c-accent,#c8ff00)' : 'var(--c-text-hint,#555568)';
    }
    textarea.addEventListener('input', _updateCounter);
    _updateCounter();

    /* ── Icon slots ── */
    const iconSection = document.createElement('div');
    iconSection.id = 'effect-icon-group';
    iconSection.style.cssText = 'display:flex;flex-direction:column;gap:6px;margin-top:8px;';

    const iconLabel = document.createElement('label');
    iconLabel.textContent = 'Effect Icons';
    iconLabel.style.cssText = 'font-size:11px;font-family:var(--font-mono,monospace);letter-spacing:0.06em;color:var(--c-text-label,#8888a0);text-transform:uppercase;';
    iconSection.appendChild(iconLabel);

    const iconHint = document.createElement('div');
    iconHint.style.cssText = 'font-size:9px;font-family:var(--font-mono,monospace);color:var(--c-text-hint,#555568);line-height:1.5;';
    iconHint.innerHTML = 'Upload square PNGs. Click <b>Insert</b> to place <code style="color:var(--c-accent,#c8ff00)">&lt;Img(N)&gt;</code> at cursor.';
    iconSection.appendChild(iconHint);

    const listEl = document.createElement('div');
    listEl.id = 'effect-icon-list';
    listEl.style.cssText = 'display:flex;flex-direction:column;gap:5px;';
    iconSection.appendChild(listEl);

    const addBtn = document.createElement('button');
    addBtn.type = 'button';
    addBtn.textContent = '+ Add Icon Slot';
    addBtn.style.cssText = [
      'align-self:flex-start', 'padding:3px 10px', 'font-size:10px',
      'font-family:var(--font-mono,monospace)',
      'background:var(--c-surface-2,#1e1e2e)', 'color:var(--c-text,#e0e0f0)',
      'border:1px solid var(--c-border,#333348)', 'border-radius:3px',
      'cursor:pointer', 'white-space:nowrap', 'line-height:1.8',
    ].join(';');
    addBtn.addEventListener('mouseover', () => addBtn.style.background = 'var(--c-surface-3,#2a2a3e)');
    addBtn.addEventListener('mouseout',  () => addBtn.style.background = 'var(--c-surface-2,#1e1e2e)');
    iconSection.appendChild(addBtn);

    section.appendChild(iconSection);

    /* ── Ensure icons array in state ── */
    if (!Array.isArray(window.NullCorps.state.effectIcons)) {
      window.NullCorps.state.effectIcons = [];
    }
    const icons = window.NullCorps.state.effectIcons;

    /* ── Helper: small button ── */
    function _mkBtn(text, title, handler, accent) {
      const b = document.createElement('button');
      b.type = 'button'; b.textContent = text; b.title = title;
      b.style.cssText = [
        'padding:2px 7px', 'font-size:9px', 'font-family:var(--font-mono,monospace)',
        accent
          ? 'background:var(--c-accent-dim,rgba(200,255,0,0.12));color:var(--c-accent,#c8ff00);border:1px solid rgba(200,255,0,0.3)'
          : 'background:var(--c-surface-2,#1e1e2e);color:var(--c-text,#e0e0f0);border:1px solid var(--c-border,#333348)',
        'border-radius:3px', 'cursor:pointer', 'white-space:nowrap',
        'line-height:1.8', 'flex-shrink:0',
      ].join(';');
      b.addEventListener('click', handler);
      return b;
    }

    /* ── Render icon list ── */
    function _renderList() {
      listEl.innerHTML = '';
      icons.forEach((icon, idx) => {
        const row = document.createElement('div');
        row.style.cssText = 'display:flex;align-items:center;gap:6px;flex-wrap:wrap;';

        const badge = document.createElement('span');
        badge.textContent = `#${idx + 1}`;
        badge.style.cssText = 'font-size:10px;font-family:var(--font-mono,monospace);color:var(--c-accent,#c8ff00);min-width:22px;flex-shrink:0;';
        row.appendChild(badge);

        const thumb = document.createElement('img');
        thumb.style.cssText = 'width:28px;height:28px;object-fit:contain;border:1px solid var(--c-border,#333348);border-radius:3px;background:#111;flex-shrink:0;';
        thumb.alt = '';
        if (icon.dataUrl) thumb.src = icon.dataUrl;
        else thumb.style.opacity = '0.2';
        row.appendChild(thumb);

        const fileLbl = document.createElement('span');
        fileLbl.textContent = icon.filename || 'no file';
        fileLbl.style.cssText = 'font-size:10px;font-family:var(--font-mono,monospace);color:var(--c-text-hint,#555568);flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';
        row.appendChild(fileLbl);

        /* Hidden file input */
        const fileInput = document.createElement('input');
        fileInput.type = 'file';
        fileInput.accept = 'image/png,image/jpeg,image/webp,image/gif,image/svg+xml';
        fileInput.style.display = 'none';
        fileInput.addEventListener('change', function () {
          const file = this.files && this.files[0];
          if (!file) return;
          const reader = new FileReader();
          reader.onload = function (e) {
            icons[idx] = { dataUrl: e.target.result, filename: file.name };
            _triggerRender();
            _renderList();
          };
          reader.readAsDataURL(file);
        });
        row.appendChild(fileInput);

        row.appendChild(_mkBtn('↑ Upload', `Upload image for icon #${idx + 1}`, () => fileInput.click(), false));

        /* Insert <Img(N)> at cursor */
        row.appendChild(_mkBtn(`Insert <Img(${idx + 1})>`, `Insert <Img(${idx + 1})> at cursor`, () => {
          const tag   = `<Img(${idx + 1})>`;
          const start = textarea.selectionStart;
          const end   = textarea.selectionEnd;
          const val   = textarea.value;
          textarea.value = val.slice(0, start) + tag + val.slice(end);
          textarea.selectionStart = textarea.selectionEnd = start + tag.length;
          textarea.focus();
          textarea.dispatchEvent(new Event('input', { bubbles: true }));
        }, true));

        /* Remove slot */
        const removeBtn = _mkBtn('✕', `Remove icon slot #${idx + 1}`, () => {
          icons.splice(idx, 1);
          _triggerRender();
          _renderList();
        }, false);
        removeBtn.style.color = 'var(--c-text-hint,#555568)';
        row.appendChild(removeBtn);

        listEl.appendChild(row);
      });
    }

    addBtn.addEventListener('click', () => {
      icons.push({ dataUrl: '', filename: '' });
      _renderList();
    });

    _renderList();
    window.NullCorps._effectIconList = icons;
  }
  /* ==============================================================
     CROSSWORD SEPARATE TOGGLE (identity section)
  ============================================================== */
  function _wireSeparateToggle() {
    const chk = document.getElementById('crossword-separate-toggle-identity');
    if (!chk) return;

    chk.checked = !!window.NullCorps.state.crosswordSeparated;

    chk.addEventListener('change', function () {
      window.NullCorps.state.crosswordSeparated = this.checked;
      const inner = document.getElementById('crossword-separate-toggle');
      if (inner && inner !== this) inner.checked = this.checked;
      if (window.NullCorps.crossword) {
        if (typeof window.NullCorps.crossword.setSeparated === 'function') {
          window.NullCorps.crossword.setSeparated(this.checked);
        } else if (typeof window.NullCorps.crossword.build === 'function') {
          window.NullCorps.crossword.build();
        }
      }
    });
  }

  function init() {
    _wireFields();
    _wireEffectEditor();
    _wireCardArt();
    _wireSeparateToggle();

    // Push any initial state values into inputs
    syncFromState();
  }

  function getIconState() {
    return (window.NullCorps.state.effectIcons || []).map(ic => ({
      dataUrl:  ic.dataUrl  || '',
      filename: ic.filename || '',
    }));
  }

  function applyIconState(snap) {
    if (!Array.isArray(snap)) return;
    window.NullCorps.state.effectIcons = snap.map(ic => ({
      dataUrl:  ic.dataUrl  || '',
      filename: ic.filename || '',
    }));
    const group = document.getElementById('effect-icon-group');
    if (group) group.remove();
    // Re-init just the icon list portion
    const section = document.getElementById('section-effect');
    const textarea = document.getElementById('card-effect');
    if (section && textarea) _wireEffectEditor();
  }

  /* -- Expose public API -- */
  window.NullCorps.editor = {
    init,
    syncFromState,
    getTextState,
    applyTextState,
    getIconState,
    applyIconState,
  };

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', init);
  } else {
    init();
  }

})();
