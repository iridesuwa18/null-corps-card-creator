...
(function () {
  ...
  function applyTerritory(territoryName) {
    ...
    // Selecting a territory snaps back to territory mode
    if (territoryName) {
      s.cardMode = 'territory';
      s.eventSubtype = '';
      _syncCardModeBtns('territory');
      _syncModeUI('territory');
    }
  ...
  function _wireEvents() {
    ...
    if (modeRow) {
      modeRow.addEventListener('click', function (e) {
        ...
        // When leaving char-skill or dozer-skill mode, coerce HP/SHD back to numbers
        if (mode !== 'char-skill' && mode !== 'dozer-skill') {
          const s = window.NullCorps.state;
        ...
        // Non-territory modes: clear territory-derived state so border/frame
  ...
  function _syncModeUI(mode) {
    ...
    if (terrSel) {
      terrSel.disabled = (mode !== 'territory');
      if (terrGroup) {
        terrGroup.style.opacity = (mode !== 'territory') ? '0.4' : '';
      }
    }
