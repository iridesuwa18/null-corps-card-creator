...
(function () {
  ...
  function _findMerge(lettersA, lettersB) {
    ...
    // 3. Any shared letter — prefer the match whose anchor position is
    //    farthest from existing merge points (avoids piling on the same spot).
    const candidates = [];
    for (let ai = 0; ai < lettersA.length; ai++) {
    ...
    // Find indices of existing merge points on the anchor word
    const mergePositions = lettersA
      .map((l, i) => (l.type === 'merge' ? i : -1))
      .filter(i => i >= 0);

    if (!mergePositions.length) return candidates[0]; // no existing merges — take first

    // Pick the candidate whose anchor index is farthest from all existing merges
  ...
  function _renderTiles() {
    ...
    // If hidden, don't render any tiles
  ...
  function _buildWordInputs(rawWords) {
    ...
    // Inject crossword controls style once (no mobile-only restriction)
    if (!document.getElementById('nc-crossword-mobile-style')) {
      const styleEl = document.createElement('style');
      styleEl.id = 'nc-crossword-mobile-style';
      styleEl.textContent = ''; // no hiding — controls always visible
      document.head.appendChild(styleEl);
    }

    /* ── Mobile-only: Locked + View controls ── */
    ...
    const lockedBtn = _mkToggleBtn('🔒 Locked', _locked, () => {
      ...
      _renderTiles(); // rewire bounding boxes with new locked state
      return _locked;
    });
  ...
  function init() {
    ...
    if (window.NullCorps.state.cardName) {
      ...
      // No card name yet — still render the settings controls (Locked, View, etc.)
      _buildWordInputs([]);
    }
  }
