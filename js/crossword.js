...
(function () {
  ...
  function _layoutWords(rawWords) {
    ...
    rawWords.forEach((wordStr, wi) => {
      ...
      // Direction: respect _flipFirst for word 0, then alternate
      let baseDir;
      if (_flipFirst) {
        baseDir = (wi % 2 === 0) ? 'v' : 'h';
      } else {
  ...
  function _buildWordInputs(rawWords) {
    ...
    sepCheck.addEventListener('change', function () {
      // Use setSeparated so both checkboxes stay in sync
    ...
    rawWords.forEach((wordStr, wi) => {
      ...
      if (!_separated) {
        ...
        mergeHint.textContent = _separated
          ? 'Special: <X>. Merge tags ignored in separate mode.'
          : 'Auto-merges on first shared letter. Use [X] to force a merge letter.';
        ...
        // Separated mode — show special-letter note, no merge hint
  ...
  /* ── Expose public API ── */
  window.NullCorps.crossword = { init, build, getState, applyState, setSeparated };
