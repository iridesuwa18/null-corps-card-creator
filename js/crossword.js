...
(function () {
  ...
  function _makeRotatable(handle, group, { onRotate }) {
    let active    = false;
    let prevAngle = null; // tracks previous raw angle to compute delta
    ...
    // Shared rotation logic: computes damped delta and calls onRotate
    function _applyRotation(clientX, clientY) {
      ...
      if (prevAngle !== null) {
        ...
        if (delta < -180) delta += 360;
        // Apply damping factor of 0.6, then snap to 0.5-degree increments
        const newAngle = Math.round((_bbState.angle + delta * 0.6) * 2) / 2;
        onRotate(newAngle);
      }
      prevAngle = raw;
    }
    ...
    window.addEventListener('mousemove', e => {
      if (!active) return;
      _applyRotation(e.clientX, e.clientY);
    });

    window.addEventListener('touchmove', e => {
      if (!active) return;
      const touch = e.touches[0];
      _applyRotation(touch.clientX, touch.clientY);
    }, { passive: false });
