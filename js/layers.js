...
(function () {
  ...
  function _isGrayMode(s) {
    ...
    return mode === 'dreamscape' || mode === 'event' || mode === 'no-territory';
  }
  ...
  const LAYER_DEFS = [
    ...
    /* No-Territory mode: hidden (no symbol shown here).                         */
    {
    ...
    /* All other modes → textbox_hpshdpanels.png                                  */
    {
      ...
      filename: (s) => _isGrayMode(s)
        ? 'textbox_hpshdpanels_gray.png'
        : 'textbox_hpshdpanels.png',
      x: CARD_W / 2, y: CARD_H / 2,
    },
  ...
  /* ══════════════════════════════════════════════════════════════
     ...
     Builds a floating popup listing all 36 layers with eye-icon
     toggles. Closes on outside click or Escape.
  ══════════════════════════════════════════════════════════════ */
  ...
  /**
   * Build the layer menu button and popup, mount into #card-panel-actions.
   */
  function _buildLayerMenu() {
    ...
    if (!layersBtn) return; // button not in HTML yet — graceful no-op

    // ── Create the popup ─────────────────────────────────────
    const popup = document.createElement('div');
    popup.id = 'layer-menu-popup';
    popup.className = 'layer-menu-popup hidden';
    popup.setAttribute('role', 'dialog');
    popup.setAttribute('aria-label', 'Layer Visibility');
    ...
    for (const def of LAYER_DEFS) {
      ...
      eyeBtn.setAttribute('aria-label', `Toggle ${def.name}`);
    ...
    // Mount popup into the card-panel so it floats over the viewport
    const cardPanel = document.getElementById('card-panel');
    if (cardPanel) cardPanel.appendChild(popup);
    ...
    function _openMenu() {
      ...
      const panelRect = cardPanel.getBoundingClientRect();
      popup.style.top  = (btnRect.bottom - panelRect.top + 6) + 'px';
      popup.style.left = Math.min(
        btnRect.left - panelRect.left,
        panelRect.width - popup.offsetWidth - 8
      ) + 'px';
    }
    ...
    layersBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      popup.classList.contains('hidden') ? _openMenu() : _closeMenu();
    });
