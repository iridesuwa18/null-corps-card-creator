...
(function () {
  ...
  function exportPNG() {
    ...
    // Layer 29 (Card Effect) is rendered by html2canvas like all other text layers.
    // The onclone computed-style stamping handles font-stretch + webkit-text-stroke,
    // and data-URI <img> icons are handled natively by html2canvas (useCORS: true).
    // Reference kept only so the finally block's restore is a safe no-op.
    const layer29El        = document.getElementById('layer-29');
    const layer29OrigStyle = layer29El ? layer29El.style.display : '';
    // (Layer 29 intentionally NOT hidden — let html2canvas capture it normally)
      ...
      .then(mod => {
        ...
        return h2c(stage, {
          ...
          onclone: (clonedDoc, clonedStage) => {
            ...
            // Strategy:
            //   • For each text layer element, stamp font/color styles
            //     directly from the LAYER_DEFS definition (not computed style)
            //     so we get the exact values we want, not what the browser resolved.
            //   • For non-layer elements (confined-inner divs, rich-text spans,
            //     icon imgs), inherit cleanly — don't stamp them individually.
            //   • Only convert -webkit-text-stroke → text-shadow on layer elements.

            for (const def of LAYER_DEFS) {
              if (def.type !== 'text') continue;

              const liveEl  = stage.querySelector(`#layer-${def.id}`);
              const cloneEl = clonedStage.querySelector(`#layer-${def.id}`);
              if (!liveEl || !cloneEl) continue;

              // Skip if layer is hidden
              if (liveEl.style.display === 'none') continue;

              const ptToPx = 3;
              const px = def.size * ptToPx;
              const isArchivo = def.font && def.font.includes('Archivo');

              // Stamp font properties directly from the layer definition
              if (isArchivo) {
                cloneEl.style.fontFamily  = "'Archivo', Arial, sans-serif";
                cloneEl.style.fontWeight  = (def.font.includes('Light')) ? '300' : '400';
                cloneEl.style.fontStretch = (def.font.includes('ExtraCondensed') || def.font.includes('Light'))
                  ? 'extra-condensed' : 'normal';
              } else {
                cloneEl.style.fontFamily  = "'TGL Engschrift', Arial, sans-serif";
                cloneEl.style.fontWeight  = 'normal';
                cloneEl.style.fontStretch = 'normal';
              }
              cloneEl.style.fontSize  = px + 'px';
              cloneEl.style.fontStyle = 'normal';

              // Stamp color from def (not computed — avoids resolving to wrong value)
              const isDozer = (window.NullCorps.state.cardMode || '') === 'dozer';
              const fc = isDozer
                ? (def.color === '#ffffff' ? '#000000' : '#ffffff')
                : def.color;
              const oc = isDozer
              ...
              cloneEl.style.color = fc;

              // Convert -webkit-text-stroke → 8-direction text-shadow
              if (isArchivo) {
                const w = def.outlineWidth;
                const shadow = [
                  `${w}px 0 0 ${oc}`,  `-${w}px 0 0 ${oc}`,
                  `0 ${w}px 0 ${oc}`,  `0 -${w}px 0 ${oc}`,
                  `${w}px ${w}px 0 ${oc}`,  `-${w}px ${w}px 0 ${oc}`,
                  `${w}px -${w}px 0 ${oc}`, `-${w}px -${w}px 0 ${oc}`,
                ].join(', ');
              ...
              // For confined layers: also stamp the confined-inner div and
              // ensure display/layout styles are preserved correctly.
              if (def.confined) {
                // Re-stamp the outer element's layout (html2canvas can struggle
                // with display:flex on a <span>; convert to block + padding trick)
                cloneEl.style.display        = 'block';
                cloneEl.style.verticalAlign  = 'middle';
                cloneEl.style.textAlign      = def.align || 'center';
                cloneEl.style.lineHeight     = (px * 1.25) + 'px';

                // Stamp the confined-inner div too
                const innerLive  = liveEl.querySelector('.confined-inner');
                const innerClone = cloneEl.querySelector('.confined-inner');
                if (innerLive && innerClone) {
                  innerClone.style.display     = 'block';
                  innerClone.style.width       = '100%';
                  innerClone.style.textAlign   = def.align || 'center';
                  innerClone.style.color       = fc;
                  innerClone.style.fontFamily  = cloneEl.style.fontFamily;
                  innerClone.style.fontWeight  = cloneEl.style.fontWeight;
                  innerClone.style.fontStretch = cloneEl.style.fontStretch;
                  innerClone.style.fontSize    = cloneEl.style.fontSize;
                  if (isArchivo) {
                    innerClone.style.webkitTextStroke = '0px transparent';
                    innerClone.style.textShadow       = cloneEl.style.textShadow;
                  }

                  // Propagate font/color down to all rich-text child spans
                  innerClone.querySelectorAll('span, strong').forEach(child => {
                    if (!child.style.fontFamily) child.style.fontFamily = cloneEl.style.fontFamily;
                    if (!child.style.fontWeight) child.style.fontWeight = cloneEl.style.fontWeight;
                    if (!child.style.fontStretch) child.style.fontStretch = cloneEl.style.fontStretch;
                    child.style.color = fc;
                    if (isArchivo) {
                      child.style.webkitTextStroke = '0px transparent';
                      child.style.textShadow       = cloneEl.style.textShadow;
                    }
                  });
                }
              }
