...
(function () {
  ...
  function _scaleStage() {
    ...
    stage.style.left = Math.round((vw - CARD_W * fitScale) / 2) + 'px';
    stage.style.top  = Math.round((vh - CARD_H * fitScale) / 2) + 'px';
  }
  ...
  /* Zoom/pan removed — card always fits to #card-viewport with no user zoom. */
  ...
  function init() {
    ...
    // Pre-load ArchivoExtraCondensed-Light so the canvas effect renderer always
    // gets the correct font, even on first render after a JSON import.
    // We kick off the load immediately and re-render once it resolves.
    if (document.fonts && document.fonts.load) {
      document.fonts.load('300 extra-condensed 1px "Archivo"').then(() => {
        render();
      }).catch(() => {
        render(); // font failed to load — render anyway with fallback
      });
    } else {
      render();
    }
