...
(function () {
  ...
  function exportPNG() {
    ...
    // ── Hide all UI-only overlay elements before capture ──────────────────
    ...
    // 2. Crossword bounding-box UI: dashed border div, resize handle, rotate handle
    //    These are direct children of .nc-crossword-group that are NOT .nc-tile
    const cwUIEls = stage.querySelectorAll(
      '.nc-crossword-group > div:not(.nc-tile), .nc-crossword-group > div[style*="se-resize"], .nc-crossword-group > div[style*="grab"]'
    );
    cwUIEls.forEach(el => el.style.display = 'none');
    // Also target by title attribute (resize/rotate handles)
    stage.querySelectorAll('.nc-crossword-group > [title="Resize"], .nc-crossword-group > [title="Rotate"]')
      .forEach(el => el.style.display = 'none');
    // Hide the dashed border overlay inside each crossword group
    stage.querySelectorAll('.nc-crossword-group > div[style*="dashed"]')
      .forEach(el => el.style.display = 'none');

    // ── Fix card-effect text for html2canvas ─────────────────────────────
    // html2canvas doesn't support -webkit-text-stroke / paint-order.
    // Temporarily swap to text-shadow outline on all Archivo-font text layers.
    const archivoLayers = stage.querySelectorAll('[id^="layer-"]');
    const _savedArchivoStyles = [];
    archivoLayers.forEach(el => {
      const cs = window.getComputedStyle(el);
      if (cs.fontFamily && cs.fontFamily.includes('Archivo')) {
        const stroke = el.style.webkitTextStroke;
        if (stroke && stroke !== 'none' && stroke !== '') {
          // Parse "Npx color" from webkitTextStroke
          const m = stroke.match(/^([\d.]+)px\s+(.+)$/);
          if (m) {
            const w = parseFloat(m[1]) / 2; // was doubled for paint-order trick; halve back
            const c = m[2].trim();
            const shadow = [
            ...
            _savedArchivoStyles.push({ el, stroke, paintOrder: el.style.paintOrder, textShadow: el.style.textShadow });
            el.style.webkitTextStroke = '0';
            el.style.paintOrder = '';
            el.style.textShadow = shadow;
          }
        }
        // Also fix inner confined divs
        el.querySelectorAll('*').forEach(child => {
          const ccs = window.getComputedStyle(child);
          if (ccs.fontFamily && ccs.fontFamily.includes('Archivo')) {
            const cs2 = child.style.webkitTextStroke;
            if (cs2 && cs2 !== 'none' && cs2 !== '') {
              const m2 = cs2.match(/^([\d.]+)px\s+(.+)$/);
              if (m2) {
                const w2 = parseFloat(m2[1]) / 2;
                const c2 = m2[2].trim();
                const shadow2 = [
                  `${w2}px 0 0 ${c2}`, `-${w2}px 0 0 ${c2}`,
                  `0 ${w2}px 0 ${c2}`, `0 -${w2}px 0 ${c2}`,
                  `${w2}px ${w2}px 0 ${c2}`, `-${w2}px ${w2}px 0 ${c2}`,
                  `${w2}px -${w2}px 0 ${c2}`, `-${w2}px -${w2}px 0 ${c2}`,
                ].join(', ');
                _savedArchivoStyles.push({ el: child, stroke: cs2, paintOrder: child.style.paintOrder, textShadow: child.style.textShadow });
                child.style.webkitTextStroke = '0';
                child.style.paintOrder = '';
                child.style.textShadow = shadow2;
              }
            }
          }
        });
    ...
    // ── Pre-warm fonts so html2canvas canvas context has them loaded ───────
    // Draw each font variant into an offscreen canvas to force the browser
    // to load them into the GPU/raster context before html2canvas runs.
    function _prewarmFonts() {
      const c = document.createElement('canvas');
      c.width = 200; c.height = 60;
      const ctx = c.getContext('2d');
      const variants = [
        { font: "400 normal 20px 'TGL Engschrift'" },
        { font: "400 normal 20px 'Archivo'" },
        { font: "400 extra-condensed 20px 'Archivo'" },
        { font: "300 extra-condensed 20px 'Archivo'" },
      ];
      variants.forEach(v => {
        ctx.font = v.font;
        ctx.fillText('Aq', 0, 40);
      });
    }
    ...
    fontsReady.then(() => {
      _prewarmFonts();
      // Small delay to let the browser settle after font warm-up and DOM changes
    ...
    .then(mod => {
      ...
      return h2c(stage, {
        ...
        logging: false,
        // Tell html2canvas to use the fonts already loaded in the document
        onclone: (clonedDoc) => {
          // Copy all @font-face stylesheet rules into the cloned document
          // so html2canvas iframe has the same fonts available
          try {
    ...
    .finally(() => {
      ...
      cwUIEls.forEach(el => el.style.display = '');
      stage.querySelectorAll('.nc-crossword-group > [title="Resize"], .nc-crossword-group > [title="Rotate"]')
        .forEach(el => el.style.display = '');
      stage.querySelectorAll('.nc-crossword-group > div[style*="dashed"]')
        .forEach(el => el.style.display = '');

      // ── Restore Archivo text styles ─────────────────────────────────────
      _savedArchivoStyles.forEach(({ el, stroke, paintOrder, textShadow }) => {
        el.style.webkitTextStroke = stroke;
        el.style.paintOrder = paintOrder;
