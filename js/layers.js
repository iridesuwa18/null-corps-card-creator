...
(function () {
  ...
  function exportPNG() {
    ...
    // 2. Crossword bounding-box UI: dashed border, resize handle, rotate handle
      ...
      .then(mod => {
        ...
        return h2c(stage, {
          ...
          onclone: (clonedDoc, clonedStage) => {
            ...
            // ── Fix every element in the clone ────────────────────────────
            // html2canvas has two known bugs with Archivo ExtraCondensed Light:
            //   1. It doesn't render -webkit-text-stroke (no outline appears)
            //   2. It doesn't resolve inherited font-weight/font-stretch on
            //      child elements — it reads only inline styles, so a child
            //      div inside a span with font-weight:300 gets weight:400.
            //
            // Fix: walk every element in the cloned stage. For each one,
            // read the LIVE computed style from the original DOM (same index),
            // then write ALL font-related properties + the text-shadow outline
            // as explicit inline styles on the clone. This bypasses inheritance
            // entirely and gives html2canvas exactly what it needs to render.
            ...
            liveEls.forEach((liveEl, i) => {
              ...
              // Stamp explicit font properties so html2canvas never has to
              // resolve inheritance.
              ...
              // html2canvas ignores -webkit-text-stroke entirely, but does
              // render text-shadow, so this is the only way to get outlines.
              const strokeWidth = parseFloat(cs.webkitTextStrokeWidth) || 0;
              if (strokeWidth > 0) {
                const strokeColor = cs.webkitTextStrokeColor || '#000000';
                // The live element uses outlineWidth*2 with paint-order to get
                // outlineWidth px of visible outside stroke. Halve it back.
