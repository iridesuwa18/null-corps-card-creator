...
(function () {
  ...
  function _scaleStage() {
    ...
    // On mobile/portrait, CSS handles the card sizing — no JS transform needed.
    // Applying a JS scale on top of the CSS layout causes the card to overflow.
    if (window.matchMedia('(orientation: portrait), (max-width: 600px)').matches) {
      // Reset any previously applied transform/position so CSS takes full control
      stage.style.transform  = '';
      stage.style.position   = 'absolute';
      stage.style.left       = '0';
      stage.style.top        = '0';
      // Compute a fit scale based on actual viewport size, apply it cleanly
