...
(function () {
  ...
  function _wireZoom() {
    ...
    /* ══════════════════════════════════════════════════════════════
       TOUCH CONTROLS
       1 finger drag  → pan  (always, regardless of zoom level)
       2 finger pinch → zoom (spread = zoom in, pinch = zoom out)
       Double-tap     → reset zoom
       All touch handling is done exclusively via touch events so
       pointer events never interfere with multi-touch gestures.
    ══════════════════════════════════════════════════════════════ */
    ...
    /* ── State ─────────────────────────────────────────────────── */
    let _panning   = false;
    let _panStartX = 0, _panStartY = 0, _panBaseOx = 0, _panBaseOy = 0;
    let _pinching  = false;
    ...
    viewport.addEventListener('touchstart', e => {
      // Don't intercept taps on buttons or overlay controls
      ...
      if (e.touches.length === 1) {
        ...
        // Switch to 2-finger pinch-zoom
        _panning        = false;
        _pinching       = true;
        _pinchDist      = _getTouchDist(e.touches);
        _pinchBaseScale = _zoom.scale;
        e.preventDefault();
      }
    }, { passive: false });
    ...
    viewport.addEventListener('touchmove', e => {
      if (e.touches.length === 1 && _panning) {
        // 1-finger pan
        _zoom.ox = _panBaseOx + (e.touches[0].clientX - _panStartX);
        _zoom.oy = _panBaseOy + (e.touches[0].clientY - _panStartY);
        _scaleStage();
        e.preventDefault();

      } else if (e.touches.length === 2 && _pinching) {
        // 2-finger zoom — scale relative to where pinch started
        const newDist   = _getTouchDist(e.touches);
        const ratio     = newDist / _pinchDist;
        _zoom.scale     = Math.max(0.5, Math.min(4, _pinchBaseScale * ratio));
        _scaleStage();
        e.preventDefault();
      }
    }, { passive: false });
    ...
    viewport.addEventListener('touchend', e => {
      ...
      if (remaining === 0) {
        // All fingers lifted — double-tap detection
        const now = Date.now();
        if (_panning && (now - _lastTapTime < 300)) _resetZoom();
        _lastTapTime = now;
        _panning  = false;
        _pinching = false;

      } else if (remaining === 1 && _pinching) {
        // One finger lifted from a pinch — resume pan with the remaining finger
        _pinching   = false;
        _panning    = true;
        _panStartX  = e.touches[0].clientX;
        _panStartY  = e.touches[0].clientY;
    ...
    let _mousePanning = false;
    let _mousePanStartX = 0, _mousePanStartY = 0, _mousePanBaseOx = 0, _mousePanBaseOy = 0;
    ...
    viewport.addEventListener('mousedown', e => {
      ...
      _mousePanBaseOx  = _zoom.ox;
      _mousePanBaseOy  = _zoom.oy;
      e.preventDefault();
    });

    window.addEventListener('mousemove', e => {
      if (!_mousePanning) return;
      _zoom.ox = _mousePanBaseOx + (e.clientX - _mousePanStartX);
      _zoom.oy = _mousePanBaseOy + (e.clientY - _mousePanStartY);
      _scaleStage();
    });
