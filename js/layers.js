...
(function () {
  ...
  function _isGrayMode(s) {
    ...
    return mode === 'dreamscape' || mode === 'event' || mode === 'no-territory' || mode === 'dozer' || mode === 'dozer-skill';
  }
  ...
  const LAYER_DEFS = [
    ...
    /* Dozer mode → stats_icon_text_dozer.png; all other modes → stats_icon_text.png */
    {
      ...
      filename: (s) => (s.cardMode === 'dozer' || s.cardMode === 'dozer-skill') ? 'stats_icon_text_dozer.png' : 'stats_icon_text.png',
      x: CARD_W / 2, y: CARD_H / 2,
    },
    ...
    /* No-Territory mode: hidden.                                                */
    {
      ...
      filename: (s) => {
        ...
        if (mode === 'dozer' || mode === 'dozer-skill') return 'dozer_symbol.png';
    ...
    /* Dozer mode → church_stained_glass_frame_dozer.png; all others → church_stained_glass_frame.png */
    {
      ...
      filename: (s) => (s.cardMode === 'dozer' || s.cardMode === 'dozer-skill') ? 'church_stained_glass_frame_dozer.png' : 'church_stained_glass_frame.png',
      x: CARD_W / 2, y: CARD_H / 2,
    },
    ...
    /*                            None → gray_leftglass.png                  */
    {
      ...
      filename: (s) => {
        ...
        if (mode === 'dozer' || mode === 'dozer-skill') return 'dozer_leftglass.png';
    ...
    /* territory: uses release-counterpart slug; hidden if no energy set.    */
    {
      ...
      filename: (s) => {
        ...
        if (mode === 'dozer' || mode === 'dozer-skill') return 'dozer_rightglass.png';
    ...
    /* All other modes → textbox_hpshdpanels.png                                  */
    {
      ...
      filename: (s) => {
        if (s.cardMode === 'dozer' || s.cardMode === 'dozer-skill') return 'textbox_hpshdpanels_dozer.png';
        if (s.cardMode === 'char-skill') return 'textbox_hpshdpanels.png';
  ...
  function render() {
    ...
    const isCharSkill = mode === 'char-skill' || mode === 'dozer-skill';
    ...
    for (const def of LAYER_DEFS) {
      ...
      // Char. Skill mode: hide layers 9 (Stats Icon), 12 (ATK), 13 (DEF)
      if (isCharSkill && (def.id === 9 || def.id === 12 || def.id === 13)) {
        el.style.display = 'none';
        continue;
      }
  ...
  function _renderText(el, def, state) {
    ...
    if (mode === 'dozer' || mode === 'dozer-skill') {
    ...
    // Char. Skill mode: HP (14) shifts left 150px, SHD (15) shifts right 150px,
    // both center-aligned. Restore defaults when switching away.
    if (def.id === 14 || def.id === 15) {
      if (mode === 'char-skill') {
        const shiftX = def.id === 14
          ? (def.x - 150)   // HP: move left 150px
          : (def.x + 150);  // SHD: move right 150px
        el.style.fontSize = '39px';
  ...
  function _scaleStage() {
    ...
    // Always use the viewport element's own clientHeight — it correctly
    // reflects the actual card-panel space available (which ResizeObserver
    // keeps up to date). visualViewport.height is the full-screen visual
    // height and would cause the card to over-scale on mobile.
    ...
    // On mobile: always fit-to-viewport with no zoom or pan — card scales to panel.
    // On desktop: apply _zoom.scale and pan offset.
  ...
  function _wireZoom() {
    ...
    // On mobile (touch screens), zoom and pan are disabled — the card always
    // fits the settings panel automatically via _scaleStage(). Return early
    // so none of the scroll/pinch/drag handlers are wired on touch devices.
    const isMobile = window.matchMedia('(pointer: coarse)').matches;
    if (isMobile) return;
