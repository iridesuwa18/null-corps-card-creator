...
(function () {
  ...
  function _isGrayMode(s) {
    ...
    return mode === 'dreamscape' || mode === 'event' || mode === 'no-territory' || mode === 'dozer' || mode === 'char-skill' || mode === 'dozer-skill';
  }
  ...
  function _renderText(el, def, state) {
    ...
    // Char. Skill mode: HP (14) shifts left 50px, SHD (15) shifts right 50px,
    // both center-aligned. Restore defaults when switching away.
    if (def.id === 14 || def.id === 15) {
      if (mode === 'char-skill') {
        const shiftX = def.id === 14
          ? (def.x - 50)   // HP: move left 50px
          : (def.x + 50);  // SHD: move right 50px
