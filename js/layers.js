...
(function () {
  ...
  function _effectFont(ctx, font, px) {
    if (font === 'tgl') {
      ...
      // Must match the @font-face declaration exactly:
      // family="Archivo", weight=300, stretch=extra-condensed.
      // "ArchivoCondLight" is not a registered family name — it causes the
      // browser to fall back to regular Archivo or the system sans-serif.
      ctx.font = `300 ${px}px "Archivo", Arial, sans-serif`;
      // font-stretch can't go in ctx.font — set it on the canvas element so
      // the browser picks the extra-condensed variant from the @font-face set.
      ctx.canvas.style.fontStretch = 'extra-condensed';
    }
  }
  ...
  function _renderEffect(canvasEl, def, state) {
    ...
    if (document.fonts && document.fonts.load) {
      const fontSpec = `300 ${fontPx}px "Archivo"`;
