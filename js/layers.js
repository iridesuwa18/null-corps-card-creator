...
(function () {
  ...
  /* ══════════════════════════════════════════════════════════════
     CARD STAGE SCALING
     Measures available space inside #card-panel (minus the actions
     bar and label bar heights), then stamps explicit px width/height
     on #card-viewport — bypassing aspect-ratio and dvh CSS entirely
     so Android chrome-bar show/hide never triggers layout thrash.
  ══════════════════════════════════════════════════════════════ */
  ...
  function _scaleStage() {
    ...
    const stage    = document.getElementById('card-stage');
    if (!panel || !viewport || !stage) return;

    const panelW = panel.clientWidth;
    const panelH = panel.clientHeight;
    if (!panelW || !panelH) return;

    // Subtract sibling bar heights from available height
    const actions = document.getElementById('card-panel-actions');
    const label   = document.getElementById('card-panel-label');
    const usedH   = (actions ? actions.offsetHeight : 0) +
                    (label   ? label.offsetHeight   : 0);
    const padding = 20; // accounts for panel gap + padding

    const availW = panelW - padding;
    const availH = panelH - usedH - padding;
    if (availW <= 0 || availH <= 0) return;

    // Fit card (2250x3150) into available space preserving ratio
    const CARD_RATIO = CARD_W / CARD_H;
    let vw, vh;
    if (availW / availH < CARD_RATIO) {
      vw = availW;
      vh = Math.round(availW / CARD_RATIO);
    } else {
      vh = availH;
      vw = Math.round(availH * CARD_RATIO);
    }

    // Lock viewport to explicit px — no more aspect-ratio or dvh recalc
    // overflow:hidden clips the zoomed stage so the browser only paints what's visible.
    // will-change:transform on the viewport creates a stacking context that contains repaints.
    viewport.style.width    = vw + 'px';
    viewport.style.height   = vh + 'px';
    viewport.style.overflow = 'hidden';
    viewport.style.position = 'relative';

    // Pre-promote stage to its own GPU compositing layer so zoom/pan are
    // compositor-only operations — no layout or paint triggered during gesture.
    stage.style.willChange  = 'transform';

    // Fit scale + user zoom
    const fitScale   = vw / CARD_W;
    const totalScale = fitScale * _zoom.scale;

    const cx = (vw - CARD_W * totalScale) / 2;
    const cy = (vh - CARD_H * totalScale) / 2;
    ...
    // translate3d keeps stage on its own GPU layer — pan/zoom = composite only
  ...
  function _wireZoom() {
    ...
    /* ── Pinch-to-zoom state — declared first so pointerdown guard can read it ── */
    let _pinchDist = null;
    ...
    // Cache viewport size for use in pointermove (avoid layout reads mid-gesture)
    let _cachedVW = 0, _cachedVH = 0;

    viewport.addEventListener('pointerdown', e => {
      // Ignore if a pinch is already active, or on UI controls
      ...
      _panBaseOx  = _zoom.ox;
      _panBaseOy  = _zoom.oy;
      // Snapshot viewport size once at drag start — no reads during move
      _cachedVW   = viewport.clientWidth;
      _cachedVH   = viewport.clientHeight;
      viewport.setPointerCapture(e.pointerId);
      viewport.style.cursor = 'grabbing';
      // No e.preventDefault() — touch-action:none on the element handles this
    }, { passive: true });
    ...
    viewport.addEventListener('pointermove', e => {
      ...
      // Use cached size — zero layout reads, pure GPU composite
      const stage = document.getElementById('card-stage');
      if (stage && _cachedVW) {
        const fitScale   = _cachedVW / CARD_W;
        const totalScale = fitScale * _zoom.scale;
        const cx = (_cachedVW - CARD_W * totalScale) / 2;
        const cy = (_cachedVH - CARD_H * totalScale) / 2;
        stage.style.transform =
          `translate3d(${cx + _zoom.ox}px,${cy + _zoom.oy}px,0) scale(${totalScale})`;
      }
    });
    ...
    viewport.addEventListener('touchstart', e => {
      if (e.touches.length === 2) {
        _panning   = false; // cancel any active pan when pinch starts
  ...
  function init() {
    ...
    // Observe #card-panel (not #card-viewport) — the panel is what changes size
    // on orientation change / chrome-bar toggle. Debounced to avoid mid-gesture thrash.
    const panelEl = document.getElementById('card-panel');
    if (panelEl && typeof ResizeObserver !== 'undefined') {
      const ro = new ResizeObserver(_debounce(_scaleStage, 50));
      ro.observe(panelEl);
    } else {
