...
(function () {
  ...
  function exportPNG() {
    ...
    // We fetch each file as a binary blob, base64-encode it, and inject
    // data-URI @font-face rules into the html2canvas cloned document.
    // This is required because html2canvas clones into about:blank, so
    // relative URLs in copied @font-face rules silently 404.
    const _FONT_DEFS = [
      ...
      .then(mod => {
        ...
        return h2c(stage, {
          ...
          onclone: (clonedDoc) => {
            ...
            // all fonts available with no network requests needed.
            if (embeddedFontCSS) {
