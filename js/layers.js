...
(function () {
  ...
  const LAYER_DEFS = [
    ...
    /* ── Layer 2: Creator Credit (Text) ── */
    {
      id: 2, name: 'iridesuwa (creator credit)', type: 'text',
    ...
    /* ── Layer 3: Null Corps (Game Name) (Text) ── */
    {
      id: 3, name: 'Null Corps (game name)', type: 'text',
    ...
    /* ── Layer 4: Unique Card Number (Text) ── */
    {
      id: 4, name: 'Unique Card Number', type: 'text',
    ...
    /* ── Layer 5: Half Gray Inner Frame ── */
    /* Visible only when Secondary Energy is an Absorb type. Hidden for Release or None. */
    {
      id: 5, name: 'Half Gray Inner Frame', type: 'png',
      filename: (s) => _isAbsorb(s.energySecondary) ? 'half_gray_inner_frame.png' : '',
      x: CARD_W / 2, y: CARD_H / 2,
    },

    /* ── Layer 6: Outer Card Outline & Inner Frame (Dynamic — Secondary Energy) ── */
    {
      id: 6, name: 'Outer Card Outline & Inner Frame', type: 'png',
    ...
    /* ── Layer 7: Half Gray Outer Frame ── */
    /* Visible only when Third Energy is an Absorb type. Hidden for Release or None. */
    {
      id: 7, name: 'Half Gray Outer Frame', type: 'png',
      filename: (s) => _isAbsorb(s.energyThird) ? 'half_gray_outer_frame.png' : '',
      x: CARD_W / 2, y: CARD_H / 2,
    },

    /* ── Layer 8: Outer Frame (Dynamic — Third Energy) ── */
    {
      id: 8, name: 'Outer Frame', type: 'png',
    ...
    /* ── Layer 9: Stats Icon & Text (Static PNG) ── */
    /* Dozer mode → stats_icon_text_dozer.png; all other modes → stats_icon_text.png */
    {
      id: 9, name: 'Stats Icon & Text', type: 'png',
      filename: (s) => (s.cardMode === 'dozer') ? 'stats_icon_text_dozer.png' : 'stats_icon_text.png',
      x: CARD_W / 2, y: CARD_H / 2,
    },

    /* ── Layers 10 & 11: Crossword Tiles — managed by js/crossword.js ── */
    /* Placeholder divs are injected here; crossword.js renders into them */
    {
      id: 10, name: 'Crossword Tiles (Horizontal)', type: 'crossword-h',
    },
    {
      id: 11, name: 'Crossword Tiles (Vertical)', type: 'crossword-v',
    },
    ...
    /* No energy selected: falls back to main_energy_diamond_gray.png.             */
    {
      id: 19, name: 'Main Energy Diamond Colour', type: 'png',
      filename: (s) => {
        if (_isGrayMode(s) || !s.energyMain) {
          return 'main_energy_diamond_gray.png';
        }
        const ds = _diamondSlug(s.energyMain);
        return ds ? `main_${ds}_energy_diamond_colour.png` : 'main_energy_diamond_gray.png';
      },
      x: CARD_W / 2, y: CARD_H / 2,
    },
    ...
    /* No energy selected: falls back to secondary_energy_diamond_gray.png.      */
    {
      id: 22, name: 'Secondary Energy Diamond Colour', type: 'png',
      filename: (s) => {
        if (_isGrayMode(s) || !s.energySecondary) {
          return 'secondary_energy_diamond_gray.png';
        }
        const ds = _diamondSlug(s.energySecondary);
        return ds ? `secondary_${ds}_energy_diamond_colour.png` : 'secondary_energy_diamond_gray.png';
      },
      x: CARD_W / 2, y: CARD_H / 2,
    },
    ...
    /* No energy selected: falls back to third_energy_diamond_gray.png.          */
    {
      id: 25, name: 'Third Energy Diamond Colour', type: 'png',
      filename: (s) => {
        if (_isGrayMode(s) || !s.energyThird) {
          return 'third_energy_diamond_gray.png';
        }
        const ds = _diamondSlug(s.energyThird);
        return ds ? `third_${ds}_energy_diamond_colour.png` : 'third_energy_diamond_gray.png';
      },
      x: CARD_W / 2, y: CARD_H / 2,
    },
  ...
  function _ensureBoundsOverlay(def) {
    ...
    // ── Settings-panel mirror: lock + hide controls for mobile ──
    // The on-canvas buttons sit at top:-22px and are clipped on mobile.
    // We inject equivalent controls into #text-box-controls-mobile so
    // they are always reachable in the scrollable settings panel.
    (function _injectSettingsControls() {
      const panel = document.getElementById('text-box-controls-mobile');
      if (!panel) return;

      // Section header — only rendered once (before the first row)
      if (!panel.querySelector('.tbc-header')) {
        const hdr = document.createElement('div');
        hdr.className = 'tbc-header';
        hdr.style.cssText = [
          'font-size:11px',
          "font-family:var(--font-mono,'Courier New',monospace)",
          'letter-spacing:.06em', 'color:var(--c-text-label,#8888a0)',
          'text-transform:uppercase', 'margin-bottom:6px',
        ].join(';');
        hdr.textContent = 'Text Box Controls';
        panel.appendChild(hdr);
      }

      // Row container
      const row = document.createElement('div');
      row.id = 'tbc-row-' + def.id;
      row.style.cssText = [
        'display:flex', 'align-items:center', 'justify-content:space-between',
        'gap:8px', 'margin-bottom:6px',
        'padding:6px 10px',
        'background:var(--c-surface-2,#1a1a22)',
        'border:1px solid var(--c-border,#2a2a2f)', 'border-radius:5px',
      ].join(';');
      ...
      // Button factory
      function _mkSettingsBtn(text, colorActive) {
        const b = document.createElement('button');
        b.type = 'button';
        b.textContent = text;
        b.style.cssText = [
      ...
      const settingsLockBtn = _mkSettingsBtn('🔒 Locked', '#f90');
      const settingsEyeBtn  = _mkSettingsBtn('👁 Hide',   '#0cf');

      // Keep settings buttons in sync with the on-canvas state
      function _syncLockBtn() {
        settingsLockBtn.textContent   = locked ? '🔒 Locked' : '🔓 Unlock';
        settingsLockBtn.style.color       = locked ? '#f90' : '#0cf';
        settingsLockBtn.style.borderColor = locked ? '#f90' : '#0cf';
      }
      function _syncEyeBtn() {
        settingsEyeBtn.textContent = visible ? '👁 Hide' : '👁 Show';
      }

      // Override _updateLock so both button sets stay in sync
      const _origUpdateLock = _updateLock;
      _updateLock = function () { _origUpdateLock(); _syncLockBtn(); };

      settingsLockBtn.addEventListener('click', () => {
        locked = !locked;
        _updateLock();          // updates both on-canvas + settings buttons
      });

      settingsEyeBtn.addEventListener('click', () => {
        visible = !visible;
        // Mirror the eyeBtn logic from the on-canvas button
        wrapper.style.borderColor = visible ? 'rgba(0,200,255,0.85)' : 'transparent';
        label.style.opacity  = visible ? '1' : '0';
        lockBtn.style.opacity = visible ? '1' : '0';
        eyeBtn.textContent   = visible ? '👁 Hide' : '👁 Show';
      ...
      row.appendChild(settingsLockBtn);
      row.appendChild(settingsEyeBtn);
  ...
  function render() {
    ...
    // (ATK=12, DEF=13, HP=14, SHD=15, TerritorySymbol=30, StatsIconText=9)
    const HIDE_IN_DREAMSCAPE_EVENT = new Set([9, 12, 13, 14, 15, 30]);
    ...
    for (const def of LAYER_DEFS) {
      ...
      if (isDreamscapeOrEvent && (def.id === 9 || def.id === 12 || def.id === 13 || def.id === 14 || def.id === 15)) {
        el.style.display = 'none';
        continue;
      }
  ...
  function _wrapEffect(ctx, segments, fontPx, maxW) {
    ...
    for (const seg of segments) {
      ...
      // Also preserve any leading whitespace — it represents a real space gap
      // between an icon/<Img> or [TGL block] and the following plain text.
      const leadingSpace = seg.text.match(/^\s+/)?.[0] ?? '';
      const chunks = seg.text.match(/\S+\s*/g) || (seg.text ? [seg.text] : []);
      for (let ci = 0; ci < chunks.length; ci++) {
        // Re-attach leading whitespace to the very first chunk so the gap
        // between e.g. <Img(1)> and "Blind" is not silently discarded.
        const text = ci === 0 ? leadingSpace + chunks[ci] : chunks[ci];
        runs.push({ type: 'text', text, font: seg.font, px: fontPx });
      }
    }
  ...
  function _effectFont(ctx, font, px) {
    if (font === 'tgl') {
      ctx.font = `normal normal ${px}px "TGL Engschrift", Arial, sans-serif`;
    } else {
      // ArchivoExtraCondensed-Light is registered under family="Archivo",
      // weight=300, stretch=extra-condensed.  Both weight AND stretch must be
      // supplied in ctx.font; omitting either causes the browser to pick the
      // wrong variant (e.g. regular Archivo or the system sans-serif).
      // font-stretch keywords are valid in the CSS font shorthand used by Canvas.
      ctx.font = `300 extra-condensed ${px}px "Archivo", Arial, sans-serif`;
      // Clear any stale fontStretch set on the canvas element — it is not needed
      // when stretch is encoded in ctx.font and can interfere on some engines.
      ctx.canvas.style.fontStretch = '';
    }
  }
  ...
  function _renderEffect(canvasEl, def, state) {
    ...
    // Guard against the font-load race: if Archivo ExtraCondensed Light hasn't
    // been used in the DOM yet (e.g. right after a JSON import), the canvas will
    // silently fall back to regular Archivo or the system font.
    // document.fonts.load() forces the font into the browser's loaded set, then
    // re-invokes this render once it's ready.
    if (document.fonts && document.fonts.load) {
      const fontSpec = `300 extra-condensed ${fontPx}px "Archivo"`;
      if (document.fonts.check(fontSpec) === false) {
        document.fonts.load(fontSpec).then(() => {
          // Re-render now that the font is available
          _renderEffect(canvasEl, def, state);
        });
        return; // Don't draw with the wrong font — wait for the callback
      }
    }
  ...
  function _scaleStage() {
    ...
    // On mobile, clientHeight can be wrong while the browser toolbar is
    // animating (collapse/expand). Use visualViewport.height when available
    // so fitScale is computed against the real visible area, not a stale value.
    ...
    // Use pointer:coarse to detect real touch screens.
    // The old (orientation:portrait) check misfired on any tall desktop window.
    const isMobile = window.matchMedia('(pointer: coarse)').matches;
    // Apply _zoom.scale on both mobile and desktop — mobile pinch-to-zoom needs it.
    // Pan offset (ox/oy) is still suppressed on mobile (pointer-drag pan is desktop-only).
  ...
  function init() {
    ...
    // On mobile, the browser toolbar animates in/out and changes the visual
    // viewport height — clientHeight won't update, but visualViewport will.
    // Wire it so the card rescales correctly after toolbar transitions.
