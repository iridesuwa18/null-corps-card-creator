...
(function () {
  ...
  function _ensureBoundsOverlay(def) {
    ...
    wrapper.style.cssText = [
      ...
      'pointer-events:all', 'cursor:not-allowed', 'z-index:' + (36 - def.id + 200),
    ].join(';');

    // ── Controls bar — lives OUTSIDE the card stage so it's never scaled down ──
    // We create a floating div anchored to the viewport, positioned by JS each frame.
    const controls = document.createElement('div');
    controls.id = 'bounds-controls-' + def.id;
    controls.style.cssText = [
      ...
      'gap:6px',
      'pointer-events:all',
      // will be positioned by _positionControls()
    ].join(';');
    ...
    label.style.cssText = 'font-size:11px;font-family:Arial,sans-serif;color:rgba(0,200,255,0.95);background:rgba(0,0,0,0.75);padding:3px 8px;border-radius:4px;white-space:nowrap;pointer-events:none;letter-spacing:0.04em;';
    controls.appendChild(label);

    function _mkBtn(text) {
    ...
    const lockBtn = _mkBtn('🔒 Locked');
    const eyeBtn  = _mkBtn('👁 Hide');

    // Position the controls bar just above the wrapper in screen space
    function _positionControls() {
      const stageEl = document.getElementById('card-stage');
      if (!stageEl) return;
      const stageRect   = stageEl.getBoundingClientRect();
      const sc          = stageEl.style.transform.match(/scale\(([^)]+)\)/);
      const scale       = sc ? parseFloat(sc[1]) : 1;
      const wLeft       = parseInt(wrapper.style.left) || 0;
      const wTop        = parseInt(wrapper.style.top)  || 0;
      const screenX     = stageRect.left + wLeft * scale;
      const screenY     = stageRect.top  + wTop  * scale;
      const ctrlH       = controls.offsetHeight || 34;
      controls.style.left = Math.round(screenX) + 'px';
      controls.style.top  = Math.round(screenY - ctrlH - 6) + 'px';
    }

    // Reposition on scroll/resize/zoom
    const _rafRepos = () => requestAnimationFrame(_positionControls);
    window.addEventListener('resize',  _rafRepos);
    window.addEventListener('scroll',  _rafRepos, true);
    // Reposition on every _scaleStage call via a MutationObserver on wrapper transform
    const _mo = new MutationObserver(_rafRepos);
    _mo.observe(stage, { attributes: true, attributeFilter: ['style'], subtree: false });
    // Initial position after paint
    requestAnimationFrame(_positionControls);
    ...
    eyeBtn.addEventListener('click', e => {
      ...
      controls.style.opacity     = visible ? '1' : '0.4';
      eyeBtn.textContent         = visible ? '👁 Hide' : '👁 Show';
    });

    // Apply initial locked state visuals
    _updateLock();
    ...
    function _startDrag(clientX, clientY, e) {
      ...
      e.preventDefault();
      e.stopPropagation(); // don't let the viewport pan handler pick this up
    }
    ...
    // Touch: only start drag if unlocked, and stopPropagation so card pan doesn't fire
    wrapper.addEventListener('touchstart', e => {
      if (locked) return; // locked = let touch fall through to card pan
