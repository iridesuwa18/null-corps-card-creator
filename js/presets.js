...
(function () {
  ...
  function _buildSnapshot(name) {
    ...
    return {
      ...
      layerVisibility: s.layerVisibility ? Object.assign({}, s.layerVisibility) : {},
    };
  }
  ...
  function _applySnapshot(snap) {
    ...
    /* 1b · Card Mode + Event Subtype */
    const mode = snap.cardMode || 'territory';
    const subtype = snap.eventSubtype || '';
    nc.state.cardMode     = mode;
    nc.state.eventSubtype = subtype;

    // Sync the mode buttons and event subtype dropdown in the UI
    if (nc.territories) {
      if (typeof nc.territories.syncCardModeBtns === 'function') {
        nc.territories.syncCardModeBtns(mode);
      }
      if (typeof nc.territories.syncModeUI === 'function') {
        nc.territories.syncModeUI(mode);
      }
    }
    // Sync event subtype dropdown value
    ...
    /* 4 · Layer visibility (Feature 5) — restore before re-render */
    if (snap.layerVisibility && typeof snap.layerVisibility === 'object') {
      nc.state.layerVisibility = Object.assign({}, snap.layerVisibility);
    } else {
      nc.state.layerVisibility = {};
    }

    /* 5 · Full layer re-render */
